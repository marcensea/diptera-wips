...
"""
    ...
    This script generates numpy files containing bases for crossvalidation
    with fair partition of the different classes represented.
    Saves preprocessed images in hdf5 format.
"""
...
images_path = Path("C://Users//Marc//Documents//Insects2022//data//Training")  #debug
...
if True:
    ...
    print("Most populated class:", wip.fullname(c), '->', populations.iat[ind])
    ind = populations.argmin()
    c = populations.index[ind]
    print("Least populated class:", wip.fullname(c), '->', populations.iat[ind])
    print("")
...
if True:
    ...
    def preprocess_images(dset, pictures, _images_path, _config):
            ...
            for batch in range(1, n_batch+1):
                ...
                for i in range(min(batch_size, N-offset)):
                    img_name = train_pictures[offset + i]
                    img_path = str(_images_path.glob('*/' + img_name + '.jpg').__next__())
                    img = cv2.imread(img_path, cv2.IMREAD_COLOR)
                    img = cv2.resize(img, (_config.image_width, _config.image_height))
                    if img_arr is None:
                        img_arr = img[None, ...]
                    else:
                ...
                print(f"Batch: {batch:03d}/{n_batch:03d} -- time elapsed: {batch_time:.1f}s -- remaining estimate: {total_time*(n_batch-batch)/batch:.1f}s")
    ...
    with h5py.File(h5py_path, 'w') as f:
        print("--- TRAINING DATA ---")
        train_dset = f.create_dataset("train", (n_train, *model_input_shape), dtype='float32', compression="gzip", compression_opts=6)
        preprocess_images(train_dset, train_pictures, images_path, config)
        print("--- TESTING DATA ---")
        test_dset = f.create_dataset("test", (n_test, *model_input_shape), dtype='float32', compression="gzip", compression_opts=6)
